...
import {UBUNDLE_REPORT} from '../../../../../constants/api.constants'
...
const ExcelExport = () => {
  ...
  const getData = async () => {
    ...
    const res: any = await getQueryRequest(`${UBUNDLE_REPORT}?${query}`)
    setLoading(false)
  ...
  if (data.length > 0 && !loading)
    return <CSVDownload target='_self' filename={'UbundleReport.csv'} data={data} />
