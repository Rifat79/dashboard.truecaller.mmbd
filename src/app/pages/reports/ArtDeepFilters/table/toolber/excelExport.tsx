...
import {ADF_REPORT} from '../../../../../constants/api.constants'
...
const ExcelExport = () => {
  ...
  const getData = async () => {
    ...
    const res: any = await getQueryRequest(`${ADF_REPORT}?${query}`)
    setLoading(false)
  ...
  if (data.length > 0 && !loading)
    return <CSVDownload target='_self' filename={'ArtDeepFilters.csv'} data={data} />
