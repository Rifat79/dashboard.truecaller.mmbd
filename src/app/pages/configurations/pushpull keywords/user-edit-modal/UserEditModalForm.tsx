...
const UserEditModalForm: FC<Props> = ({user, isUserLoading}) => {
  ...
  const [userForEdit] = useState<User>({
    ...
    brandKeyword: user?.brandKeyword || initialUser.brandKeyword,
  })
      ...
      try {
        if (isNotEmpty(itemIdForUpdate)) {
          const res: any = await updateUser({
            id: itemIdForUpdate,
            brandKeyword: values.brandKeyword,
            organizationId: values.organization.id,
            status: values?.status?.id,
            filter_id: itemIdForUpdate?.id
          });
          if(res?.data?.success) {
            cancel(true);
          } else {
        ...
        >
          <div id='kt_modal_update_customer_user_info' className='show row row-cols-1 row-cols-sm-2 row-cols-lg-2 gy-4'>
            ...
            <div className='fv-row mb-3'>
              ...
              <input 
                type='text' 
                placeholder='brandKeyword'
                {...formik.getFieldProps('brandKeyword')}
                className='form-control' 
                name='brandKeyword' 
              />
              {formik.touched.brandKeyword && formik.errors.brandKeyword && (
                  <div className='fv-plugins-message-container'>
                    <div className='fv-help-block'>
                      <span role='alert'>{formik.errors.brandKeyword}</span>
                    </div>
                  </div>
                )}
            </div>
