...
const DateRangeMaker = ({ onChange, hideTitle = false, className = '' }) => {
    ...
    const onOkay = (data, item = {}) => {
        ...
        if (onChange)
            onChange({
                start_date: `${moment(data.startDate).format('yyyy-MM-DD')} 00:00:00`,
                end_date: `${moment(data.endDate).format('yyyy-MM-DD')} 23:59:59`,
                selected: item,
            })
    ...
    return (
        <>
            {!hideTitle && <label className='form-label fs-6 fw-bold'>Date Range:</label>}
            <div className='time position-relative min-w-150px' id='date-range-ref'>
                <button
                    type='button'
                    className={`d-flex form-select ${className || 'form-select-solid'
